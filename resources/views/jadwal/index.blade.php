            ...
            <div class="card-body">
                ...
                <form method="GET" class="d-flex align-items-center">
                    ...
                    <select name="perPage1" id="perPage" onchange="this.form.submit()"
                        ...
                        <option value="8" {{ request('perPage1') == 8 ? 'selected' : '' }}>8</option>
                        <option value="30" {{ request('perPage1') == 30 ? 'selected' : '' }}>30</option>
                        <option value="50" {{ request('perPage1') == 50 ? 'selected' : '' }}>50</option>
                        <option value="{{ $jadwalsuntik->count() }}" {{ request('perPage1') == $jadwalsuntik->count() ? 'selected' : '' }}>Semua</option>
                    </select>
                    </div>
                </form>
                ...
                <table class="table table-hover">
                    ...
                    <tr>
                        ...
                        <td>{{ $pengingat->jadwalPengingat }}</td>
                        <td>{{ $pengingat->jenisPengingat }}</td>
                        <td>
                        <div class="d-flex gap-2">
                            <a href="{{ route('jadwal.edit', $pengingat->id) }}" class="btn btn-warning btn-sm me-2">
                            <i class="fas fa-edit"></i>
                            </a>
                            <form action="{{ route('jadwal.destroy', $pengingat->id) }}" method="POST" class="m-0 ms-2">
                            @csrf
                            @method('DELETE')
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center p-3 bg-light rounded mb-3">
                        <form method="GET" class="d-flex align-items-center">
                            <div class="input-group">
                                ...
                                <select name="perPage1" id="perPage" onchange="this.form.submit()"
                                    class="form-select border-start-0" style="width: 100px;">
                                    <option value="10" {{ request('perPage1') == 10 ? 'selected' : '' }}>8</option>
                                    <option value="30" {{ request('perPage1') == 30 ? 'selected' : '' }}>30</option>
                                    <option value="50" {{ request('perPage1') == 50 ? 'selected' : '' }}>50</option>
                                    <option value="{{ $jadwalperiksa->count() }}" {{ request('perPage1') == $jadwalperiksa->count() ? 'selected' : '' }}>Semua</option>
                                </select>
                            </div>
                        </form>
                        <a href="{{ route('jadwal.periksa') }}" class="btn btn-success btn-lg shadow-sm">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover">
                            ...
                            <tbody>
                                @forelse($jadwalperiksa as $periksaa)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ $periksaa->namaPasien }}</td>
                                    <td>{{ $periksaa->noTelepon }}</td>
                                    <td>{{ $periksaa->tanggalPeriksa }}</td>
                                    <td>{{ $periksaa->jadwalPengingat }}</td>
